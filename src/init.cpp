...
{
    uiInterface.ThreadSafeMessageBox(str, _("TrollCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("TrollCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 22001 or testnet: 19117)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 22000)") + "\n" +
...
bool AppInit2()
{
    ...
    // TrollCoin: Keep irc seeding on by default for now.
//    if (fTestNet)
//    {
...
//    }
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  TrollCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    // requested to kill TrollCoin-qt during the last operation. If so, exit.
